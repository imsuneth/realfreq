...
echo_bams | ex ./realfreq -d test/tmp/dump.tmp -r test/tmp/genome_chr22.fa -o test/tmp/test1.tsv >> test/tmp/test1.log || die "Test 1 failed running realfreq"
...
echo_bams | ex ./realfreq -d test/tmp/dump.tmp -r test/tmp/genome_chr22.fa -o test/tmp/test2.bedmethyl -b >> test/tmp/test2.log || die "Test 2 failed running realfreq"
...
echo "test/example-ont.bam" | ex ./realfreq -d test/tmp/test3.dump.tmp -r test/tmp/genome_chr22.fa -o test/tmp/test3.out1.tsv >> test/tmp/test3.log1.log || die "Test 3a failed running realfreq - run"
echo "Test 3b: loading"
echo "" | ex ./realfreq -d test/tmp/test3.dump.tmp -s -r test/tmp/genome_chr22.fa -o test/tmp/test3.out2.tsv >> test/tmp/test3.log1.log || die "Test 3b failed running realfreq - resume"
