...
echo_bams | ex ./realfreq test/tmp/genome_chr22.fa -d test/tmp/test1.dump.tmp -o test/tmp/test1.tsv >> test/tmp/test1.log || die "Test 1 failed running realfreq"
...
echo_bams | ex ./realfreq test/tmp/genome_chr22.fa -d test/tmp/test2.dump.tmp -o test/tmp/test2.bedmethyl -b >> test/tmp/test2.log || die "Test 2 failed running realfreq"
...
echo "test/example-ont.bam" | ex ./realfreq  test/tmp/genome_chr22.fa -d test/tmp/test3.dump.tmp -o test/tmp/test3.out1.tsv >> test/tmp/test3.log1.log || die "Test 3a failed running realfreq - run"
echo "Test 3b: loading"
echo "" | ex ./realfreq test/tmp/genome_chr22.fa -d test/tmp/test3.dump.tmp -o test/tmp/test3.out2.tsv -r >> test/tmp/test3.log2.log || die "Test 3b failed running realfreq - resume"
