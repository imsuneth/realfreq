...
(
    ...
    ${MINIMAP2} -t 8 -ax map-ont --sam-hit-only -Y -y --secondary=no $REFIDX $FASTQ_FILEPATH > $UNSORTED_BAM_FILEPATH 2>> $LOG_FILEPATH || failed "minimap2 failed. check $LOG_FILEPATH" $P5_FILEPATH
    t5=$(date)
    ...
    ${F5C} index --slow5 $SLOW5_FILEPATH $FASTQ_FILEPATH >> $LOG_FILEPATH 2>&1 || failed "f5c index failed. check $LOG_FILEPATH" $P5_FILEPATH
    t8=$(date)
    echo -e "$P5_FILEPATH\tf5c-index\t${t7}\t${t8}" >> ${TIME_LOG}

    ${F5C} call-methylation --slow5 $SLOW5_FILEPATH -b $BAM_FILEPATH -g $REF -r $FASTQ_FILEPATH > $F5C_FILEPATH 2>> $LOG_FILEPATH || failed "f5c call-methylation failed. check $LOG_FILEPATH" $P5_FILEPATH
    t9=$(date)
